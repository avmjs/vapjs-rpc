...
function VapRPC(cprovider, options) {
  ...
  if (!(this instanceof VapRPC)) { throw new Error('[vapjs-rpc] the VapRPC object requires the "new" flag in order to function normally (i.e. `const vap = new VapRPC(provider);`).'); }
  ...
  self.setProvider = (provider) => {
    if (typeof provider !== 'object') { throw new Error(`[vapjs-rpc] the VapRPC object requires that the first input 'provider' must be an object, got '${typeof provider}' (i.e. 'const vap = new VapRPC(provider);')`); }
  ...
  const promise = new Promise((resolve, reject) => {
    self.currentProvider.sendAsync(parsedPayload, (err, response) => {
      ...
      if (err || responseObject.error) {
        const payloadErrorMessage = `[vapjs-rpc] ${(responseObject.error && 'rpc' || '')} error with payload ${JSON.stringify(parsedPayload, null, self.options.jsonSpace)} ${err ? String(err) : (JSON.stringify(responseObject.error, null, self.options.jsonSpace))}`;
